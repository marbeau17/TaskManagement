...
test.describe('TC-04: Task Creation', () => {
  ...
  test('shows step indicator', async ({ page }) => {
    ...
    // The labels are in the title attribute of the circle divs
    const step1Circle = page.locator('[title="依頼情報入力"]')
    const step2Circle = page.locator('[title="アサイン設定"]')
  ...
  test('shows info notice bar', async ({ page }) => {
    await expect(page.locator('text=依頼情報を入力して送信すると')).toBeVisible({ timeout: 15000 })
  })
  ...
  test('shows validation errors for empty required fields', async ({ page }) => {
    ...
    if (submitCount > 0) {
      ...
      const clientError = page.locator('text=クライアント名は必須です')
      const titleError = page.locator('text=タスク名は必須です')
